...
export const ReservationForm: React.FC<{ onCreated?: () => void }> = ({
  onCreated,
}) => {
    ...
    const run = async () => {
      ...
      try {
        const res = await api.get("/vehicles", {
          params: { plate, status: "available", limit: 1 },
        });
        const items = res.data?.items || res.data || [];
        const v = items.find((x: any) =>
  ...
  // 🔹 Crear reserva con garantes
  const handleCreate = async () => {
    if (!client)
      return alert(
        "Cliente no encontrado. Verificá el DNI o cargalo en el módulo de Clientes."
      );
    if (!vehicle)
      return alert("La patente indicada no está disponible o no existe.");
    ...
    // Si se cargaron datos de garantes, los sube
    const hasAnyGuarantorData = guarantors.some(
      (g) =>
        g.firstName ||
        g.lastName ||
        g.dni ||
    ...
    if (hasAnyGuarantorData) {
      for (const g of guarantors) {
        if (!(g.firstName && g.lastName && g.dni)) continue;
        await addGuarantor(
          created.id,
          {
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 3, background: "#1e1e2f" }}>
      ...
      {guarantors.map((g, idx) => (
        <Paper
          key={idx}
          sx={{ p: 2, mb: 2, background: "#24243a", borderRadius: 2 }}
        >
          <Grid container spacing={2} columns={12}>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                value={g.firstName}
                onChange={(e) =>
                  handleChangeGuarantor(idx, "firstName", e.target.value)
                }
                fullWidth
                sx={{ input: { color: "#fff" }, label: { color: "#ccc" } }}
              />
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                value={g.lastName}
                onChange={(e) =>
                  handleChangeGuarantor(idx, "lastName", e.target.value)
                }
                fullWidth
                sx={{ input: { color: "#fff" }, label: { color: "#ccc" } }}
              />
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                value={g.dni}
                onChange={(e) =>
                  handleChangeGuarantor(idx, "dni", e.target.value)
                }
                fullWidth
                sx={{ input: { color: "#fff" }, label: { color: "#ccc" } }}
              />
            ...
            <Grid item xs={12} md={5}>
              <TextField
                ...
                value={g.address}
                onChange={(e) =>
                  handleChangeGuarantor(idx, "address", e.target.value)
                }
                fullWidth
                sx={{ input: { color: "#fff" }, label: { color: "#ccc" } }}
              />
            ...
            <Grid item xs={12} md={3}>
              <TextField
                ...
                value={g.phone}
                onChange={(e) =>
                  handleChangeGuarantor(idx, "phone", e.target.value)
                }
                fullWidth
                sx={{ input: { color: "#fff" }, label: { color: "#ccc" } }}
              />
